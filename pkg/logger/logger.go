...
func NewLogger(loggerType string, serviceKV map[string]interface{}, hookCore zapcore.Core) (*zap.SugaredLogger, error) {
	...
	logonce.Do(func() {
		...
		case "prod":
			...
			if err != nil {
				log.Fatalf("Error initializing sugared zap logger: %s", err)
			}
		case "dev":
		...
		default:
			log.Fatalf("Unknown logger type: %s", loggerType)
		}
		if hookCore != nil {
...
func WithContext(ctx context.Context, ctxFields []CtxKeyType) *zap.SugaredLogger {
	if Log == nil {
		// TODO: Valid level must be passed or else this is invalid invocation.
		NewLogger("", nil, nil)
	}
